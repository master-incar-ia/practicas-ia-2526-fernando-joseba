...
class ConvolutionalNeuralNetwork(nn.Module): # Hereda de nn.Module, es un requisito
    """
    Modelo de red convolucional con 2 capas ocultas para exercise_05:
    ...
    def __init__(self, output_dim, num_hidden_neurons): # Parametros de salida y número de neuronas en las capas ocultas
        ...
        self.dropout = nn.Dropout(p=0.6) # Dropout para evitar el overfitting
        self.fc2 = nn.Linear(num_hidden_neurons, output_dim)

    def forward(self, x):

        # Bloque 1
        x = F.relu(self.conv1(x)) # Primera transformación lineal (Conv2d) + activación ReLU
        x = self.pool1(x) # MaxPool para reducir el tamaño de la imagen a la mitad (de 32x32 a 16x16)

        # Bloque 2
        x = F.relu(self.conv2(x)) # Segunda transformación lineal (Conv2d) + activación ReLU
        x = self.pool2(x) # MaxPool para reducir el tamaño de la imagen a la mitad (de 16x16 a 8x8)

        # Clasificador
        x = self.flatten(x) # Aplanar la imagen de 3D a 1D para que pueda ser procesada por las capas lineales
        x = F.relu(self.fc1(x)) # Tercera transformación lineal (Linear) + activación ReLU
        x = self.dropout(x) # Dropout para evitar el overfitting
...
if __name__ == "__main__": # Para probar si el modelo funciona
    model = ConvolutionalNeuralNetwork(output_dim=10, num_hidden_neurons=64) # CIFAR-10 tiene 10 clases, y se elige un número de neuronas en las capas ocultas
    print(model)
    x = torch.randn(1, 3, 32, 32) # Crear un tensor de prueba con el tamaño de una imagen CIFAR-10 (1 imagen, 3 canales, 32x32 píxeles)
    print(model(x))
    pass